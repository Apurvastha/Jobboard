...
JobBoard Team
        ...
        logger.info(
            f"Application received email send to {company_email} for job: {job_title}"
        )
...
def send_status_change_email(self, application_id, old_status, new_status):
    ...
    try:
        ...
        status_messages = status_messages.get(
            new_status, f"has been updated to {new_status}"
        )
    ...
    try:
        ...
        for company in companies_with_pending.select_related("user"):
            ...
            pending_count = Application.objects.filter(
                job__company=company, status="pending", applied_at__lt=three_day_ago
            ).count()
