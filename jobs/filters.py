...
class JobListingFilter(django_filters.FilterSet):
    # exact match filters
    job_type = django_filters.ChoiceFilter(choices=JobListing.JobType.choices)
    ...
    posted_after = django_filters.DateFilter(
        field_name="posted_at", lookup_expr="date__gte"
    )
    posted_before = django_filters.DateFilter(
        field_name="posted_at", lookup_expr="date__lte"
    )
