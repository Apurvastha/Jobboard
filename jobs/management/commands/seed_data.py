...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        backend = Category.objects.get_or_create(name="Backend", slug="backend")[0]
        frontend = Category.objects.get_or_create(name="Frontend", slug="frontend")[0]
        ai = Category.objects.get_or_create(name="AI/ML", slug="ai-ml")[0]
        devops = Category.objects.get_or_create(name="DevOps", slug="devops")[0]
        ...
        for username, email in candidate_data:
            ...
            CandidateProfile.objects.get_or_create(
                user=user,
                defaults={
                    "bio": "Backend engineer looking for opportunities in Japan.",
        ...
        JobListing.objects.bulk_create(bulk_jobs, ignore_conflicts=True)

        # add random tags to bulk jobs
        for job in JobListing.objects.filter(title__startswith="Engineer Position"):
        ...
        blog_career = BlogCategory.objects.get_or_create(name="Career", slug="career")[
            0
        ]
        blog_python = BlogCategory.objects.get_or_create(name="Python", slug="python")[
            0
        ]
